...
const REASON_LABEL: Record<DiffReason, string> = {
  kind: 'type différent (fichier / dossier)',
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center" onMouseDown={close}>
      ...
      >
        <div className="flex shrink-0 items-center gap-2 border-b border-border px-4 py-3">
          ...
          <span className="text-[13px] font-medium text-fg">Comparer les volets</span>
          {result && (
            <span className="min-w-0 truncate text-[11px] text-fg-muted">
              {baseName(a.path)} ⟷ {baseName(b.path)} · {result.identical} identique
              {result.identical > 1 ? 's' : ''}
            </span>
          )}
          <button
            onClick={() => useNavStore.getState().refreshAll()}
            title="Actualiser les deux volets"
        ...
        {!result ? (
          ...
          <div className="min-h-0 flex-1 overflow-auto p-3 text-[12px]">
            <SectionTitle
              ...
              label={`Uniquement dans ${baseName(a.path) || a.path}`}
            ...
            <SectionTitle
              ...
              label={`Uniquement dans ${baseName(b.path) || b.path}`}
            ...
            <SectionTitle
              ...
              label="Présents des deux côtés mais différents"
            ...
            <p className="mt-4 px-1 text-[11px] text-fg-muted">
              Comparaison superficielle du niveau courant (les sous-dossiers ne sont pas
              parcourus ; deux dossiers de même nom sont considérés identiques).
            </p>
          </div>
        )}
      </div>
...
function Empty(): JSX.Element {
  return <p className="px-2 py-1 text-fg-muted">Rien.</p>
}
...
function DiffRow({ row }: { row: CompareRow }): JSX.Element {
  const detail = (e: DirEntry): string =>
    e.kind === 'file' ? `${fmtBytes(e.size)} · ${formatDate(e.modifiedMs)}` : 'dossier'
  return (
    <div className="rounded-app px-2 py-1 hover:bg-bg-hover" title={REASON_LABEL[row.reason]}>
