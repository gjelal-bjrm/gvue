...
les dossiers. **Vue liste ou grille** (vignettes, bascule barre d'outils), colonne
**Type** triable. **Restauration de session** : colonnes, onglets et dossiers
rouverts au démarrage (option, activée par défaut). **Conflits de copie** gérés
façon explorateur : Remplacer (l'existant part à la corbeille) / Ignorer / Garder
les deux, avec comparaison taille/date. **Commandes personnalisées** (palette →
« Commandes personnalisées… ») : actions de menu contextuel exécutées dans le
terminal intégré, jetons `{path}` `{dir}` `{name}` `{stem}` `{ext}`.
**Parcourir une archive** (clic droit sur un zip/7z…) : contenu listé sans
extraction (7-Zip, ou .NET pour les zip) + « Extraire tout ». **Chemins réseau (UNC)** : saisir `\\serveur` dans la barre d'adresse
...
sinon colle le presse-papiers. **Terminaux liés aux onglets** (bouton 🔗, persisté) :
chaque terminal retient l'onglet de dossier qui l'a créé — en mode lié, le panneau
n'affiche que les terminaux de l'onglet actif et bascule avec lui (ceux d'un onglet
fermé redeviennent globaux). **Recherche dans le terminal** (**Ctrl+F**, Entrée /
...
- **Phase 6 (début) — Pro**
  ...
  - Intégrations applications (détectées par chemin d'install) : ouvrir avec VS Code, éditer avec Notepad++, compresser/extraire avec 7-Zip (interface graphique `7zG.exe` → progression native) — regroupées en **sous-menus** (« Ouvrir avec », « 7-Zip »). **« Applications Windows… »** ouvre la boîte système « Ouvrir avec » (toutes les applications du registre) ; **« Propriétés »** ouvre la boîte Propriétés de l'explorateur.
